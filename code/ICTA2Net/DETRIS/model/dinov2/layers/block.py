...
class Block(nn.Module):
    def __init__(
        ...
        if use_adapter:        
            self.adapter = DenseAligner(dim, visual_adapter_dim, visual_adapter_dim//2, visual_adapter_dim//16, visual_adapter_dim//4, visual_adapter_dim//16, visual_adapter_dim//4) #fc_in_channels, in_channels, ch1x1, ch3x3in, ch3x3out, ch5x5in, ch5x5out
            drop_path = 0.
            self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()


    def forward(self, x: Tensor, text_features = None) -> Tensor:
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, text_features=None):

        if isinstance(x_or_x_list, Tensor):
            return super().forward(x_or_x_list, text_features)
