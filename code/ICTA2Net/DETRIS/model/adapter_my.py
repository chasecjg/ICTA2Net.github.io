...
class DenseAligner(nn.Module):
    ...
    def forward(self, x: Tensor, text_features,split_token=0) -> List[Tensor]:
...
if __name__=="__main__":
    #fc_in_channels, in_channels, ch1x1, ch3x3in, ch3x3out, ch5x5in, ch5x5out
    dim = 768
    visual_adapter_dim = 128
    model = DenseAligner(dim, visual_adapter_dim, visual_adapter_dim//2, visual_adapter_dim//16, visual_adapter_dim//4, visual_adapter_dim//16, visual_adapter_dim//4) 
    vis = torch.randn(48, 49, 768)
    txt = torch.randn(48, 77, 512)
    # torch.Size([48, 261, 768])
    # torch.Size([48, 77, 512])
    out = model(vis, txt)
    print(out.shape)