...
# from .segmenter_score import DETRIS
# from .segmenter_mamba import DETRIS
...
#         else:
#             fix.append(v)
#     logger.info('Backbone with decay={}'.format(len(backbone)))
#     param_list = [{
#         'params': backbone,
#         'initial_lr': args.lr_multi * args.base_lr
#     }]
    ...
    for k, v in model.named_parameters():
        if (k.startswith('txt_backbone') and 'positional_embedding' not in k  or 'dinov2' in k) and v.requires_grad:
            ...
            if 'neck' in k:
                ...
                score.append(v)
            elif 'text_attention' in k:
                text_attention.append(v)
            elif 'contrative' in k:
                contrative.append(v)
            elif 'mamba' in k:
                mamba.append(v)
        else:
